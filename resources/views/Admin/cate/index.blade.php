...
<div class="mws-panel-body no-padding">
   <form action="#" method="get">
    ...
    <tbody role="alert" aria-live="polite" aria-relevant="all">
            ...
            @foreach($data as $v)
            <tr class="odd" >
                <td class=" " style="border:1px #CCCCCC solid;">{{$v->id}}</td>
                ...
                <td class=" " style="border:1px #CCCCCC solid;width: 140px;">
                    <form style="display: inline;" action="/admin/cate/{{$v->id}}" method="post">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <input type="submit" value="删除" class="btn btn-danger" onclick="confirm('确定删除吗?')">
                    </form>
                    <form style="display: inline;" action="/admin/cate/{{$v->id}}/edit" method="get">
                        {{ csrf_field() }}
